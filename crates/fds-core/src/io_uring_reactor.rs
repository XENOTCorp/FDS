...
impl IoUringReactor {
    ...
    pub fn submit_read(&mut self, fd: i32, buf: &mut [u8], user_data: u64) -> std::io::Result<()> {
    ...
    pub fn submit_write(&mut self, fd: i32, data: &[u8], user_data: u64) -> std::io::Result<()> {
    ...
    pub fn drain<F: FnMut(u64, std::io::Result<u32>)>(&mut self, f: F) -> usize {
        let _ = f;
