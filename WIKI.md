...
- its datapath: epoll edge-triggered and event-driven with the syscall
  transports by default (an explicit busy-poll spin is available for
  dedicated cores), or the io_uring completion-driven datapath;
...
- Event-driven: `wait` blocks in the kernel until an event is ready,
  waking at most every 100 ms to observe the stop flag. Zero idle CPU.
- Busy-poll (opt-in, `reactor.busy_poll=true`): `wait(timeout 0)`
  repeats until the ready list is empty. The engine burns CPU when
  idle; that is the latency contract for a dedicated core.
...
- Disable interrupt coalescing on the datapath queue:
  ...
  `adaptive-rx off adaptive-tx off`. With busy-poll enabled the engine
  spins on the ready queue; interrupts then add latency.
