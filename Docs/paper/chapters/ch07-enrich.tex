...
central idea is that Mol is a \emph{resource-sensitive monoidal category}:
every molecule carries a cost vector with components for execution time,
...
The chapter is the quantitative backbone of the thesis. \autoref{thm:25} makes the cost
enrichment well-defined: additive composition cost, the triangle inequality,
and tensor subadditivity. \autoref{thm:26} reduces global optimality to shortest paths in
the finite type graph (Dijkstra, complete). \autoref{thm:27} proves the fusion bound:
fusing costs no more than composing, and strictly less when the intermediate
type never materializes. \autoref{thm:28} and \autoref{thm:29} formalize refinement and fast-path
...
cost-minimal. \autoref{thm:30} closes the complexity class of the dataplane: atoms whose
costs lie in a class closed under the resource operations yield molecules in
that class. \autoref{thm:31} is the descriptive-complexity anchor: finite-state molecules
realize exactly the regular behaviors, and minimal-state molecules are exactly
the minimal DFAs \cite{nerode}\cite{hopcroft}. \autoref{thm:32} is projection reduction:
...
\autoref{thm:25} shows that this assignment is well-defined (independent of the order in
which the clauses are applied), additive over composition, and subadditive
over tensor.
\end{definition}

\setcatalog{25}
\begin{theorem}[Cost enrichment is well-defined]\label{thm:25}
 Fix an atom cost assignment for a signature $\Sigma$
...
equivalence: behaviorally equivalent molecules may carry different costs (\autoref{thm:6}
...
Cost is additive along composition (\autoref{thm:25}), so the cost of a pipeline that is a
...
\setcatalog{26}
\begin{theorem}[Global optimality via shortest paths]\label{thm:26}
 Let $\Sigma$ be a finite signature with bounded types and
nonnegative atom costs, additively extended (\autoref{thm:25}). The \emph{type graph} $G$
...
composite. By additivity (\autoref{thm:25}),
...
its components' costs (\autoref{thm:25}), and such parallelism is absorbed into the
signature by batch atoms (\autoref{thm:17}, \autoref{ch:operad}, \autoref{thm:47}). The theorem's
...
for it. The lookup table of \autoref{ch:free} (\autoref{thm:24}) stores exactly the
shortest paths of each behavior class.
\end{remark}

\section{The fusion bound}\label{sec:enrich-fusion}

Product-state composition fuses a pipeline into a single morphism (\autoref{thm:49},
...
\setcatalog{27}
\begin{theorem}[Fusion bound]\label{thm:27}
 Let $f \colon A \to B$ and $g \colon B \to C$ be molecules.
...
atoms, \autoref{thm:25}), so its nominal cost is $c (f) + c (g)$ and, since it never
...
cost-nonincreasing (\autoref{thm:18}). Together with the fusion bound they imply that the
...
(\autoref{thm:24}, \autoref{thm:26}, \autoref{thm:29}).
\end{remark}
...
\setcatalog{28}
\begin{theorem}[Refinement substitution]\label{thm:28}
 Refinement is preserved by substitution into compositions.
...
the pairs involved (in particular under the additive extension, \autoref{thm:25}).
\end{theorem}
...
congruence for $\circ$ and $\otimes$ (\autoref{thm:6}), so $g \circ f \simeq g' \circ f'$
and $f \otimes g \simeq f' \otimes g'$.
\emph{Cost monotone, sequential.} By additivity (\autoref{thm:25}),
...
$c (f \otimes g) = c (f) + c (g)$ and $c (f' \otimes g') = c (f') + c (g')$ (\autoref{thm:25}),
...
checked once against the general one (behavioral equivalence, \autoref{thm:6} and
\autoref{ch:testing}), and \autoref{thm:28} lifts the guarantee to every composite.
\end{remark}

\setcatalog{29}
\begin{theorem}[Maximal refinement]\label{thm:29}
 Let $\mathcal{C}$ be a finite set of molecules
...
normalization is cost-nonincreasing (\autoref{thm:18}), so the normal form of a class is a
refinement of every enumerated representative (\autoref{thm:22}--\autoref{thm:24}), and fusion (\autoref{thm:27})
...
\setcatalog{30}
\begin{theorem}[Complexity closure]\label{thm:30}
 (1) \emph{Structural closure.} Let $P$ be a property of
...
(\autoref{thm:3}, \autoref{ch:mol}), and the free-category construction of
...
\emph{(2): resource bounds by induction, using the composition laws of \autoref{thm:25}.}
...
discipline of \autoref{ch:linear} (\autoref{thm:50}) is the engineering form of this
...
\setcatalog{31}
\begin{theorem}[Descriptive complexity of Mol]\label{thm:31}
 (1) \emph{Regularity.} Every finite-state molecule
...
deterministic Mealy machine (\autoref{thm:8}, \autoref{ch:mol}), so its transduction is a
...
state-space bijection quotient of \autoref{ch:mol} (\autoref{thm:2}) the minimal molecule
...
$\simeq$, unique up to state-space isomorphism (\autoref{thm:38}, \autoref{ch:coalgebra}).
...
\setcatalog{32}
\begin{theorem}[Projection reduction]\label{thm:32}
 Let $M = (S, \mathit{step})$ be a finite-state molecule and
...
word on every input word: they are behaviorally equivalent (\autoref{thm:6}).
...
(\autoref{thm:38}, \autoref{ch:coalgebra}): is a quotient of the fiber quotient
...
the state-space bijection quotient of Mol (\autoref{thm:2}) the isomorphism is an equality
of Mol-morphisms.
\end{proof}
...
\section{Resource-Sensitive Cost Enrichment}\label{sec:enrich-graded}

The cost enrichment of \autoref{thm:25}--\autoref{thm:27} accounts for a
molecule by a single cost number. Real dataplane cost has structure:
which cache lines a molecule touches, whether concurrently written hot
fields collide in a line (false sharing), and how often the branch
predictor guesses wrong. This section refines the enrichment to a
\emph{resource-graded} structure: a graded monad whose grades live in
a resource monoid: and shows that the optimality theorems of
\ref{thm:26}--\ref{thm:27} survive the refinement.

\begin{definition}[Resource grades]\label{def:grades}
Let $M = \mathbb{N} \times \mathbb{N} \times \mathbb{R}_{\ge 0}$ be the
\emph{resource monoid}, with elements $r = (L, F, B)$ and componentwise
addition and order. For a molecule $f \colon A \to B$: $L (f)$ is the
number of distinct cache lines its working set touches (a structure of
size $s$ occupies $\lceil s/64 \rceil$ lines at the standard 64-byte
line granularity); $F (f)$ is the number of \emph{false-sharing penalty
events}: a penalty event occurs when a write to a hot field of $f$
hits a cache line also touched by a concurrently written hot field of
another molecule running alongside it; and $B (f) = \sum_i p_i \pi_i$ is
the \emph{expected branch-misprediction cost}, the sum over branch
points of $f$ of the misprediction probability $p_i$ times the penalty
$\pi_i$ in cycles. The \emph{grade} of $f$ is $g (f) = (L (f), F (f),
B (f)) \in M$.
\end{definition}

\setcatalog{53}
\begin{theorem}[Resource-graded cost enrichment, \autoref{thm:53}]\label{thm:53}
The assignment $f \mapsto g (f)$ of \autoref{def:grades} is a
well-defined enrichment of Mol over the monoidal category $(M, +)$:
\begin{enumerate}
\item[(i)] the identity has grade $0$ and composition is additive:
$g (\mathrm{id}_A) = (0,0,0)$ and $g (h \circ f) = g (f) + g (h)$;
\item[(ii)] tensor grades are subadditive in the cache-line component
with ceiling arithmetic and additive in the other components:
$L (f \otimes h) \le L (f) + L (h)$, with equality when the working sets of
$f$ and $h$ are disjoint; $F (f \otimes h) = F (f) + F (h)$ when the hot
fields of $f$ and $h$ lie in distinct cache lines and
$F (f \otimes h) = F (f) + F (h) + 1$ when they share a line;
$B (f \otimes h) = B (f) + B (h)$;
\item[(iii)] the induced pseudo-metric $d (A,B) = \inf\{\, g (f)_1 +
g (f)_2 + g (f)_3 : f \colon A \to B \,\}$ satisfies the triangle
inequality.
\end{enumerate}
Consequently the shortest-path theorem \ref{thm:26} extends: with grades
additive, a composition from $A$ to $B$ minimizing the total grade is
exactly a shortest path in the finite type graph whose edge weights are
the grades, and any locally minimal molecule: one no single-step
refinement of which lowers the total grade: is globally optimal.
\end{theorem}

\begin{proof}
\emph{(i)} The identity molecule is empty: it touches no cache lines,
has no concurrently written hot fields, and has no branch points, so
$g (\mathrm{id}_A) = (0,0,0)$. For composition, the accounted costs are
additive by definition of the account: the composite $h \circ f$ is
accounted to touch the lines of $f$ and the lines of $h$ ($L (h \circ f)
= L (f) + L (h)$; the actual number of distinct lines touched is at most
this account, so the account is a conservative bound), its false-sharing
account is the sum of the components' accounts, and its branch points
are the disjoint union of the branch points of $f$ and of $h$, so by
linearity of expectation
\[ B (h \circ f) = \sum_{i \in \mathrm{bp} (f)} p_i \pi_i +
\sum_{j \in \mathrm{bp} (h)} p_j \pi_j = B (f) + B (h), \]
with no independence hypothesis required: expectation is linear.

\emph{(ii)} A structure of size $s$ occupies $\lceil s/64 \rceil$ lines,
so the tensor $f \otimes h$ (working sets of sizes $s_f$, $s_h$)
occupies at most $\lceil (s_f + s_h)/64 \rceil \le \lceil s_f/64 \rceil +
\lceil s_h/64 \rceil = L (f) + L (h)$ lines, with equality when the
working sets are disjoint. The false-sharing account adds a collision
term: the hot fields of $f \otimes h$ are the pairs of hot fields of the
components; a penalty event arises exactly when a pair shares a cache
line, which contributes $0$ when the components' hot fields are
separated by the layout (the hot/cold separation of the standard's
[CACHE] policy) and $1$ otherwise. The branch component is additive by
linearity of expectation, as in (i), over the disjoint union of branch
points.

\emph{(iii)} For any $f \colon A \to B$ and $h \colon B \to C$, the
composite $h \circ f \colon A \to C$ has grade $g (f) + g (h)$ by (i), so
\[ d (A,C) \le g (f) + g (h) \quad\text{for all } f, h. \]
Taking the infimum over $f$ and $h$ on the right gives $d (A,C) \le
d (A,B) + d (B,C)$: the triangle inequality. (The three grade components
combine additively along a path, so the componentwise order makes the
infimum well-defined and the inequality holds componentwise.)

\emph{Extended shortest-path.} By (i), the grade of a composition is the
sum of the grades of its atomic steps, so the total grade of a molecule
is the weight of its path in the type graph. Minimizing the grade is
therefore a shortest-path problem in the finite type graph (finite
whenever the type signature is bounded, as in \ref{thm:22}), solvable by
Dijkstra's algorithm with the extraction-order induction of \ref{thm:26}.
For local optimality: suppose a molecule $M \colon A \to B$ is not
globally optimal, so some $M'$ has strictly smaller total grade. The
composite paths from $A$ to $B$ form a finite graph; by additivity,
moving from $M$ to $M'$ along a cheaper route decomposes into a finite
sequence of single-step grade decreases, the first of which is a
one-step refinement of $M$ with strictly lower grade: contradicting
local minimality. Hence local minima are global optima.
\end{proof}

\begin{remark}
The three components license the three layout disciplines of the
companion standard: $L$ is what the cache-line accounting of [CACHE]
minimizes; $F$ is exactly the quantity the hot/cold separation and
per-core data-structure policies drive to zero; and $B$ is the quantity
the refinement poset of \ref{thm:28}--\ref{thm:29} orders, so that the
most-refined dispatch structure (dense integer matches, branchless
cmov patterns) is precisely the one minimizing expected branch cost.
The graded enrichment is a graded monad in the sense that grades
compose additively along Kleisli-like composition; the resource
annotation is the type-level shadow of the runtime account.
\end{remark}
...
\midrule
\autoref{thm:25} & [N] & cost enrichment well-defined: additivity, triangle inequality, tensor subadditivity \\
\autoref{thm:26} & [N] & optimal molecules are shortest paths in the type graph; Dijkstra complete \\
\autoref{thm:27} & [N] & fusion bound $c (h^{*}) \le c (f) + c (g)$; strict when the intermediate never materializes \\
\autoref{thm:28} & [N] & refinement substitution; behavior preserved, cost monotone \\
\autoref{thm:29} & [N] & unique most-refined element per class; dispatcher sound and cost-minimal \\
\autoref{thm:30} & [N] & complexity closure under $\circ$ and $\otimes$ \\
\autoref{thm:31} & [C] & finite-state molecules = regular behaviors; minimal states = minimal DFA \\
\autoref{thm:32} & [N] & projection reduction $S_{\min} \cong \pi (S)$ \\
\bottomrule
\end{tabular}
...
enrichment of the category (\autoref{thm:25}): composition adds, tensor is subadditive,
...
algorithm is complete for it (\autoref{thm:26}). Fusion always pays: the fused composite
costs no more than the sum of its parts, and strictly less when the
intermediate type never materializes (\autoref{thm:27}). Refinement gives fast-path
specialization a soundness certificate: substitution preserves behavior and
monotonically improves cost (\autoref{thm:28}), and the most-refined dispatcher is sound
and cost-minimal (\autoref{thm:29}). Complexity closes over composition: the resource
class of the dataplane is the resource class of its atoms (\autoref{thm:30}). The
expressive ceiling is exactly the regular level of the hierarchy (\autoref{thm:31}), and
projection reduction prunes state that is only consulted through a projection
(\autoref{thm:32}). Together with the normal forms of \autoref{ch:free} and the decision
...
computable problem (\autoref{thm:51}).
