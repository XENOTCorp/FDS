...
\texttt{contraction} & \ref{thm:43} & Bound arithmetic: $d_0 = d (x_0, F x_0)$; for sampled $(\alpha, \varepsilon, d_0)$ with $\alpha < 1$, computes $k (\alpha,\varepsilon,d_0) = \lceil \ln (\varepsilon (1-\alpha)/d_0)/\ln\alpha\rceil$ and asserts the geometric iterate stays within $\varepsilon$ after $k$ steps.\\
...
\texttt{affine\_typer} & \ref{thm:55} & Linear combinator calculus: sequential composition of morphisms and the tensor bifunctor; every variable consumed exactly once; contraction and weakening rejected; evaluation preserves the leaf multiset and never increases the node count (syntax tree, not dataplane heap).\\
\bottomrule
\end{tabular}
\end{table}
