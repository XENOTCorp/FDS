\chapter{Delayed Feedback and Fixed-Iteration Loops}\label{ch:trace}

The symmetric monoidal structure of \autoref{ch:mol} describes how molecules are joined; delayed feedback describes how a molecule is turned back on itself by a register. This chapter develops that operator and its engineering content: the reactor loop of \autoref{def:reactor}, the laws proved from the machine definition in \autoref{ch:kleisli} (vanishing, sliding of pure maps, superposing, and delay in place of yanking), and the \emph{fixed-iteration loops} of a dataplane: loops whose trip count is decided in advance, unrolled, and therefore branch-predictable.

The quantitative heart of the chapter is contraction. A loop whose state update shrinks distances converges to a unique fixed point at a geometric rate (\autoref{thm:42}, Banach \citep{banach}); the iteration bound \autoref{thm:43} bounds the number of unrolled iterations needed to guarantee a tolerance $\varepsilon$; \autoref{thm:44} shows how contraction rates combine under the two composition schemes of Mol, tensor and sequential; and \autoref{thm:45} extends the analysis to stochastic state updates, where total-variation distance contracts by the Dobrushin coefficient \citep{dobroushin}. The chapter closes with the information-geometric reading of coordinate choice, recorded as future work.
...
\begin{remark}
The construction is total and deterministic, hence a well-defined morphism of Mol. Vanishing, superposing, and sliding of pure maps are proved from this definition as \autoref{thm:34}--\autoref{thm:36} in \autoref{ch:kleisli}; the same proofs show $\mathrm{Tr} (\sigma_{U,U}) = \Delta_U$, the one-step delay, so yanking fails. Sliding is the hoisting law: a pure transformation moves across the feedback boundary and out of the loop.
\end{remark}
...
\setcatalog{42}
\begin{theorem}[Contraction state update, \autoref{thm:42} {[}C{]}]\label{thm:42}
...
\setcatalog{43}
\begin{theorem}[Iteration bound, \autoref{thm:43} {[}N{]}]\label{thm:43}
Let $(S,d)$, $F$, $\alpha < 1$, and $x^{*}$ be as in \autoref{thm:42}. Let $x_0 \in S$ with $d_0 = d (x_0,Fx_0) > 0$ and let $\varepsilon > 0$ with $\varepsilon (1-\alpha) < d_0$. Define
...
\begin{proof}
By the tail bound of \autoref{thm:42}, $d (x_k,x^{*}) \leq d_0\,\alpha^k/(1-\alpha)$. This is at most $\varepsilon$ exactly when $\alpha^k \leq \varepsilon (1-\alpha)/d_0$. Since $\ln \alpha < 0$, taking logarithms gives
...
\begin{remark}[Script verification]\label{rem:thm43-verified}
The bound arithmetic of \autoref{thm:43} is \emph{script-verified}: the executable Rust tool \texttt{contraction} of \autoref{app:a} computes $k (\alpha,\varepsilon,d_0)$ with $d_0 = d (x_0, F x_0)$ and simulates the geometric model $x_{n+1} = \alpha x_n$ on the line (fixed point $0$, so $d (x_0, F x_0) = (1-\alpha) \lvert x_0 \rvert$ and $x_0 = d_0 / (1-\alpha)$). For this map the a priori bound is tight: $\lvert x_k \rvert = d_0\,\alpha^k/(1-\alpha)$. For all samples ($\alpha \in \{0.1,\ldots,0.9\}$, $\varepsilon \in \{10^{-3},10^{-6},10^{-9}\}$, $d_0 \in \{1,10\}$) it asserts $\lvert x_k \rvert \leq \varepsilon$ (relative tolerance $10^{-12}$), and it asserts the monotonicity of $k$ in $\alpha$ and in the tightening of $\varepsilon$. All 54 samples pass; the log is \texttt{verify/logs/contraction.log}.
\end{remark}

\section{Contraction Rates under Composition}\label{sec:trace-compose}

Loops are joined by the two composition schemes of Mol; a bound on the combined contraction rate is what makes the iteration bound \autoref{thm:43} compositional.

\setcatalog{44}
\begin{theorem}[Contraction rates combine, \autoref{thm:44} {[}N{]}]\label{thm:44}
...
\item[(ii)] \emph{Sequential.} On a common state space $S$ with metric $d$, the composite $F_2 \circ F_1$ is a contraction with rate $\alpha_1\alpha_2$. The product is sharp: affine maps $x \mapsto \alpha_i x$ compose to rate exactly $\alpha_1\alpha_2$. The sum $\alpha_1 + \alpha_2$ may exceed $1$ even when both factors are contractions (e.g.\ $\alpha_1 = \alpha_2 = 0.6$), and is therefore discarded as a sequential rate.
\end{enumerate}
\end{theorem}
...
For the second claim: the probability distributions on $S$ form a complete metric space under total variation, and $\mu \mapsto \mu K$ is a contraction with rate $\delta (K) < 1$. By \autoref{thm:42} it has a unique fixed point $\pi$, the stationary distribution. Inducting on the first claim,
...
\begin{remark}
A deterministic state update $F$ has Dirac kernels $K (x,\cdot) = \delta_{F (x)}$, and its Dobrushin coefficient is $1$ whenever $F$ is not constant: matching the observation that on a discrete state space the only nontrivial contractions are constant maps. The contractive regime $\delta (K) < 1$ is therefore genuinely stochastic: it arises for mixtures of deterministic updates, for example when the register is drawn from a distribution or the update depends on a randomized input. \autoref{thm:43} then bounds the number of distributional iterations, with $\alpha = \delta (K)$. This is the point of contact between the loop analysis of this chapter and the Markov-category viewpoint of \citep{arxiv-markov-categories}: where a Markov category composes stochastic kernels, the fixed-iteration analysis of Mol bounds their convergence.
\end{remark}

\section{Information Geometry as Future Work}\label{sec:trace-geometry}

The contraction rate of a state update is a property of the chosen metric: rescaling a register, or passing to adapted coordinates, changes $\alpha$ and therefore the iteration bound of \autoref{thm:43}. Choosing coordinates that minimize $k (\alpha,\varepsilon,d_0)$ is an optimization over metrics, and the geometry of the resulting loop-state manifolds (curvature, geodesics, and the divergence with respect to which the update contracts) has an information-geometric reading. Working this out, together with its interaction with \autoref{thm:43} and \autoref{thm:44}, is future work.

\section{Summary}\label{sec:trace-summary}

Feedback in Mol is the reactor loop of \S\ref{sec:trace-reactor}: a delayed trace, with vanishing, superposing, sliding of pure maps, and delay-in-place-of-yanking proved as \autoref{thm:34}--\autoref{thm:36} in \autoref{ch:kleisli}. A dataplane loop is unrolled for a fixed trip count, and this chapter proves the trip count is a theorem: convergence to a unique fixed point (\autoref{thm:42}); the closed form $k (\alpha,\varepsilon,d_0) = \lceil \ln (\varepsilon (1-\alpha)/d_0)/\ln\alpha \rceil$ with $d_0 = d (x_0, F x_0)$, script-verified (\autoref{thm:43}); the composed rates $\max (\alpha_1,\alpha_2)$ under tensor and $\alpha_1\alpha_2$ sequentially (\autoref{thm:44}); and Dobrushin contraction for stochastic updates (\autoref{thm:45}). A fixed-iteration loop is branch-predictable by construction, within a provable tolerance of its converged value, and closed under the composition of molecules.
