...
\setcatalog{24}
\begin{theorem}[Completeness for the runtime theory, and the lookup table]\label{thm:24}
 Let $R$ be the runtime rewrite system of
\autoref{ch:rewrite} on the molecules over a signature $\Sigma$,
terminating and confluent (\autoref{thm:13}--\autoref{thm:17}). Write
$=_R$ for the equational theory of $R$ (the smallest congruence
containing the rules). Then for all molecules $M, M'$ over $\Sigma$:
\[
M =_R M' \quad\text{iff}\quad \mathit{NF} (M) = \mathit{NF} (M'),
\]
and $\mathit{NF} (M) =_R M$ for every $M$. Soundness for behavior:
$M =_R M'$ implies $M \approx M'$ (\autoref{thm:18}). The converse,
that every behavioral equivalence is an $R$-equation, fails in
general: two atoms that realize the same function are behaviorally
equivalent and need not be $R$-equal. If $\Sigma$ is finite with
bounded types and bounded atom count, the normal forms form a finite,
precomputable lookup table of $R$-classes, one entry per runtime
equivalence class.
\end{theorem}

\begin{proof}
\emph{Decision of $=_R$.} Termination and confluence of $R$
(\autoref{thm:13}--\autoref{thm:17}) give a unique normal form for every
term, and $M =_R M'$ if and only if $M$ and $M'$ have a common reduct,
if and only if $\mathit{NF} (M) = \mathit{NF} (M')$. In particular
$M \to^{*} \mathit{NF} (M)$ yields $M =_R \mathit{NF} (M)$.
\emph{Soundness for $\approx$.} Each rule preserves behavior
(\autoref{thm:18}), so $=_R$ is contained in $\approx$.
\emph{Failure of the converse.} Let $f, g : A \to B$ be two distinct
atoms of $\Sigma$ whose Set-interpretations coincide. Then $f \approx g$
by equality of responses, while $\mathit{NF} (f) = f \neq g =
\mathit{NF} (g)$ because neither atom is a redex of $R$. Hence $\approx$
strictly contains $=_R$ as soon as $\Sigma$ has two extensionally equal
atoms.
\emph{Lookup table.} By \autoref{thm:22} the normal forms of bounded
molecules are finite in number, and $M \mapsto \mathit{NF} (M)$ is a
bijection between $=_R$-classes and normal forms. The table is one
$R$-normal molecule per runtime class, indexed by the type pair, finite,
and precomputed by exhaustive normalization. The \texttt{normal\_forms}
tool (\autoref{app:a}) enumerates, on the signature $f \colon A \to B$,
$g \colon B \to C$, $h \colon C \to D$, all well-typed terms with at most
six atom occurrences, and checks that equality of normal forms agrees
with equality of the realized functions on that finite fragment: PASS on
the sampled interpretation, which is completeness of $R$ for that
concrete Set-model, not for $\approx$ on an arbitrary signature.
\end{proof}
