...
(\autoref{thm:9}--\autoref{thm:12}), the parser theory of sequence and choice
...
\]
which read, in the composition notation of \autoref{thm:9}, as
...
\begin{theorem}[Modular completion, \autoref{thm:54} [N]]\label{thm:54}
...
\emph{Confluence.} By Newman's lemma \ref{thm:14}, local confluence and
...
confluence yield confluence by \ref{thm:14}. Completeness follows.

\emph{(iii)} By (i) or (ii) the union is complete, so by the argument
of \ref{thm:19} two terms are equal in $E_1 \cup E_2$ if and only if
