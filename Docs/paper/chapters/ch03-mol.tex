...
product type, and $()$ is the unit type. Sequential composition is written
$g \circ f$ for ``run $f$, then $g$''; the pipeline notation $f ; g$ means
$g \circ f$.
...
\textbf{[C].} Composition of $(S, \mathrm{step})$ classes is associative with
...
\textbf{[C].} The state-space bijection quotient is well defined: equivalent
...
$\alpha\bigl ((), ((a, b), c)\bigr) = \bigl ((a, (b, c)), ()\bigr)$; the left and right unitors
$\lambda_A : () \times A \to A$ and $\rho_A : A \times () \to A$ are the pure
projection machines, $\lambda\bigl ((), ((), a)\bigr) = (a, ())$ and
$\rho\bigl ((), (a, ())\bigr) = (a, ())$. Each is an isomorphism in Mol, its inverse
...
\textbf{[C].} PureMol is equivalent to the category \textbf{Type} of types
...
\textbf{[C].} EffMol$(\mathrm{Ctx})$ is isomorphic to the Kleisli category of
...
Fix a context type $\mathrm{Ctx}$. Every morphism of Mol falls into
exactly one of three classes, read off the state space of any
representative.

\begin{definition}[Hybrid molecules]\label{def:hybridmol}
Let $M = [S, \mathrm{step}]$ be a morphism of Mol and let $\mathrm{Ctx}$
be a fixed context type. Call $M$
\begin{itemize}
  \item \emph{pure} when $S \cong ()$,
  \item \emph{effectful over} $\mathrm{Ctx}$ when $S \cong \mathrm{Ctx}$,
  \item \emph{hybrid} otherwise.
\end{itemize}
The three classes are pairwise disjoint whenever $\mathrm{Ctx} \not\cong
()$. Write $P$, $E$, and $H$ for the three classes. Hybrid is the
residual class: it includes machines whose state is a product
$S_{\mathrm{pure}} \times \mathrm{Ctx}$ with $|S_{\mathrm{pure}}| > 1$,
and it includes machines whose state is not of that form.
\end{definition}

\begin{remark}
The classes $P$ and $E$ are the morphisms of PureMol and of
EffMol$(\mathrm{Ctx})$ respectively, the latter compared by function
equality rather than by Mol composition. Mol composition of two
effectful representatives has state space $\mathrm{Ctx} \times
\mathrm{Ctx}$, which is isomorphic to $\mathrm{Ctx}$ only in degenerate
cases, so $E \circ E \subseteq H$ in Mol. Kleisli composition of
EffMol threads a single copy of $\mathrm{Ctx}$ and satisfies
$E \circ_K E \subseteq E$; the two operations must not be identified
(\autoref{thm:67}, \autoref{thm:5}).
\end{remark}
...
\textbf{[C].} Behavioral equivalence (bisimulation) is a congruence for
...
\emph{Tensor is a congruence.} The tensor machine takes input words over
the product alphabet $A \times C$: a word
$w = ((a_1, c_1), \ldots, (a_n, c_n))$ is a lockstep pair of words of
equal length. Writing $\pi_A (w) = (a_1, \ldots, a_n)$ and
$\pi_C (w) = (c_1, \ldots, c_n)$,
\[ r_{f \otimes g, (s,t)} (w)
   = \mathrm{zip}\bigl (r_{f,s} (\pi_A (w)),\, r_{g,t} (\pi_C (w))\bigr) , \]
where $\mathrm{zip}$ pairs the two output words of length $n$ letterwise.
The response set of $f \otimes g$ is therefore determined by the pair
$(R_f, R_g)$: every lockstep word projects to a pair of component words,
and every pair of equal-length component words arises as a projection.
Equality of the component response sets gives
$f \otimes g \approx f' \otimes g'$. Hence $\approx$ is a congruence for
both operations. Independent words of unequal length are not inputs of
the tensor; they are inputs of a pair of separate machines.
...
\textbf{[C].} PureMol and EffMol are closed under the tensor: the tensor of
...
\textbf{[C].} Pure molecules and tensor products of pure molecules are
...
\midrule
\ref{thm:1} & [C] & Mol is a category: composition of $(S, \mathrm{step})$ classes is associative with identity. \\
\ref{thm:2} & [C] & The state-space bijection quotient is well defined: equivalent machines compose to equivalent machines. \\
\ref{thm:3} & [C] & Mol is symmetric monoidal: tensor the tuple product, unit $()$, coherent braiding \citep{maclane}. \\
\ref{thm:4} & [C] & PureMol is equivalent to the category of types and total functions. \\
\ref{thm:5} & [C] & EffMol$(\mathrm{Ctx})$ is isomorphic to the Kleisli category of the state monad on $\mathrm{Ctx}$. \\
\ref{thm:6} & [C] & Behavioral equivalence is a congruence for $\circ$ and $\otimes$; Mol$/{\approx}$ is symmetric monoidal. \\
\ref{thm:7} & [C] & PureMol and EffMol are closed under the tensor; $E \otimes E \subseteq H$. \\
\ref{thm:8} & [C] & Pure molecules and tensor products of pure molecules are deterministic; behavioral equivalence preserves determinism. \\
\bottomrule
\end{tabular}
...
\textbf{[N].} Let $\Sigma = \Sigma_p \uplus \Sigma_e$ be an atom
signature partitioned into pure and effectful atoms, and let
$F(\Sigma)$ be the free symmetric monoidal category on $\Sigma$,
interpreted in Mol. There is exactly one assignment
$c : \mathrm{Mor}(F(\Sigma)) \to \{P, E, H\}$ of the three classes of
\autoref{def:hybridmol} that (i) agrees with the partition on atoms,
(ii) assigns the structural morphisms (identities, braidings, unitors,
associators) to $P$, and (iii) is a congruence for Mol composition and
tensor, with the closure rules
\begin{align*}
P \circ P &\subseteq P, &
P \circ E &\subseteq E, &
E \circ P &\subseteq E, &
E \circ E &\subseteq H,\\
P \otimes P &\subseteq P, &
P \otimes E &\subseteq E, &
E \otimes P &\subseteq E, &
E \otimes E &\subseteq H,
\end{align*}
and $H \circ {\cdot} \subseteq H$, ${\cdot} \circ H \subseteq H$,
$H \otimes {\cdot} \subseteq H$, ${\cdot} \otimes H \subseteq H$.
The class map is the one of \autoref{def:hybridmol}: $c (f) = P$ when
the state space is isomorphic to $()$, $c (f) = E$ when it is
isomorphic to $\mathrm{Ctx}$, and $c (f) = H$ otherwise. Every morphism
of the free model lands in one of the three classes; no fourth class is
generated. Kleisli composition of EffMol is a different operation, and
satisfies $E \circ_K E \subseteq E$; Kleisli composition lies outside
the operations of the congruence.
\end{theorem}

\begin{proof}
On representatives, Mol composition has state space $T \times S$ and
Mol tensor has state space $S \times T$ (\autoref{def:comp},
\autoref{def:tensor}). The four sequential rules are then the
arithmetic of isomorphisms $() \times () \cong ()$,
$() \times \mathrm{Ctx} \cong \mathrm{Ctx} \cong \mathrm{Ctx} \times
()$, and $\mathrm{Ctx} \times \mathrm{Ctx} \not\cong \mathrm{Ctx}$ in
general, so two effectful machines compose to a hybrid. The four
tensor rules are the same arithmetic. Any product involving a hybrid
state space remains hybrid. These identities hold on classes by
\autoref{thm:2}.
...
and tensors, apply the closure rules. The assignment is total because
every morphism of the free category is built from atoms, structural
morphisms, $\circ$, and $\otimes$. It agrees with
\autoref{def:hybridmol} on the interpretation in Mol, because the
interpretation sends generators to machines of the declared class and
preserves $\circ$ and $\otimes$. Well-definedness on the equational
theory of $F(\Sigma)$: the symmetric monoidal axioms equate morphisms
whose state spaces are in bijection by structural isomorphisms of
tuples, and those bijections preserve the three classes (they never
identify $()$ with $\mathrm{Ctx}$ or a hybrid state with either, for
$\mathrm{Ctx} \not\cong ()$). Uniqueness: any assignment satisfying
(i)--(iii) agrees with $c$ on atoms and structural morphisms, and then
agrees on every composite by induction on syntax, because (iii) forces
the class of each composite to be the closure-rule value of the
classes of its factors. Totality of the three-valued map is the ``no
fourth class'' claim. The parenthetical Kleisli operation $\circ_K$ is
the composition of \autoref{def:effmol}, which threads one
$\mathrm{Ctx}$ rather than forming a product, and therefore lies outside
the operations of $F(\Sigma)$ named in (iii).
\end{proof}

\setcatalog{68}
\begin{theorem}[Universality: every Mealy machine is a molecule]\label{thm:68}\label{thm:universality}
\textbf{[C]/[N].}
(1) \textbf{[C].} For any symmetric monoidal category $\mathcal{C}$ and
any interpretation of the signature $\Sigma$ in $\mathcal{C}$, there is
a unique strict symmetric monoidal functor $F(\Sigma) \to \mathcal{C}$
extending the interpretation \citep{maclane}.
(2) \textbf{[N].} Every Mealy machine $(S, \mathrm{step})$ with
$\mathrm{step} : S \times A \to B \times S$ is already a molecule: it
is the representative of the morphism $[S, \mathrm{step}] : A \to B$ of
Mol (\autoref{def:mol}). In particular every finite-state Mealy
transformation is a morphism of Mol, with no additional atoms and no
feedback construction required.
(3) \textbf{[C].} Objects of Mol are types, so finite products and
coproducts of objects lift from the ambient type category. Pairing of
morphisms $f : C \to A$ and $g : C \to B$ is
$(f \otimes g) \circ \Delta_C$ with $\Delta_C : C \to C \otimes C$ the
pure copy map. Copairing of $f : A \to C$ and $g : B \to C$ is the
machine with state $S_f \times S_g$ that cases on $A + B$ and leaves the
idle factor unchanged. Delayed feedback is the reactor loop of
\autoref{def:reactor} (\autoref{thm:35}). Batching is the operad of
\autoref{ch:operad} (\autoref{thm:46}). Structural equality in
$F(\Sigma)$ is decided by Mac Lane's coherence theorem \citep{maclane}.
\end{theorem}

\begin{proof}
(1) is the universal property of the free symmetric monoidal category:
$F(\Sigma)$ is initial among symmetric monoidal categories equipped
with a $\Sigma$-indexed family of morphisms, and the unique functor
interprets generators as generators, composites as composites, and
coherence isomorphisms by the coherence theorem.

(2) By \autoref{def:machine} a Mealy machine is a pair
$(S, \mathrm{step})$, and by \autoref{def:mol} a morphism of Mol is an
isomorphism class of such pairs. The class $[S, \mathrm{step}]$ is the
required molecule. Finite-state is the special case where $S$ is
finite. No auxiliary signature is required: the machine is the
morphism.

(3) Finite products of types are the tensor of \autoref{thm:3}; the
pure copy $\Delta_C$ exists because types are the types of a cartesian
universe (\autoref{ch:prelim}). Copairing as stated is a well-defined
machine, total on $A + B$, and the two inclusions of $A$ and $B$
recover $f$ and $g$ up to the idle-state bijection that discards a
fixed unused factor. Delayed feedback and batching are
\autoref{def:reactor} and the $n$-batch of \autoref{ch:operad}.
Coherence is Mac Lane's theorem, as in the last paragraph of the proof
of \autoref{thm:3}.
\end{proof}
...
monad is exactly EffMol (\autoref{sec:mol-eff}); Freyd categories and
Hughes arrows are abstract machines with a premonoidal tensor, while
Mol's tensor is a true product, which is what licenses the interchange
and coherence laws of \autoref{sec:mol-mon}; coalgebraic final
semantics underlies the minimization theorems of
\autoref{ch:coalgebra}; effectus theory targets probabilistic and
quantum effects, outside the deterministic fragment used here; and
process calculi introduce nondeterminism, which would invalidate the
cost and allocation theorems of \autoref{ch:enrich} and
\autoref{ch:linear}. The trichotomy of \autoref{thm:classification}
records Mol composition, in which two effectful machines become
hybrid. The Kleisli-closed table $E \circ_K E \subseteq E$ describes
EffMol, a different category.
\end{remark}
