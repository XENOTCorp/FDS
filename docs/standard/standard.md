...
**Atoms.** An *atom* is a transformation with no internal composition — the smallest unit out of data paths are built. Two kinds exist. A **pure atom** is a total function `f : A → B`: same input, same output, no observable effect (thesis NT4: PureMol is equivalent to the category of types and total functions). An **effectful atom** is an arrow of EffMol(Ctx), the Kleisli category of the state monad over an effect context Ctx (NT5): it computes `f : A → B` while touching exactly the capabilities that Ctx names — I/O handles, clocks, allocation — and nothing else. An atom's contract declares its input domain (a subset of A, possibly all of A), its output type B, and — if effectful — its context Ctx.

**State space and step function.** A stateful transformation is a pair `(S, step)` with `step : S × A → B × S` (NT1): given state `s ∈ S` and input `a ∈ A`, it produces output `b ∈ B` and next state `s′ ∈ S`. The state space S is the complete, declared memory of the transformation; there is no other state (policy A-02).

**Molecules.** A *molecule* is a stateful transformation `M : A → B`, i.e., a morphism of the category Mol — formally, an equivalence class of pairs `(S, step)` quotiented by the state-space bijection condition (NT2). A **pure molecule** lives in PureMol and computes without effects; pure molecules and tensor products of pure molecules are deterministic (NT4, NT8). An **effectful molecule** acts within a context Ctx (NT5). A **hybrid molecule** mixes pure and effectful steps; every hybrid molecule decomposes uniquely as `M = q ∘ e ∘ p`, where `p` and `q` are pure and `e` is effectful (NT33). Molecules compose sequentially by **∘** and in parallel by **⊗** (the tensor is the tuple product; unit is `()`; NT3). Composition is associative (NT1), preserves behavioral equivalence (NT6), and lifts refinement with cost monotonicity (NT28).

**Behavioral equivalence.** Two molecules are *behaviorally equivalent*, written `M ≈ M′`, when they produce indistinguishable input/output behavior — bisimulation, a congruence for ∘ and ⊗ (NT6). Every molecule has a **normal form** `NF(M)` under the runtime's equational theory: `NF(M) ≈ M`, `NF(M)` is unique up to coherence, and equivalence is decidable by comparing normal forms (NT23, NT24; the full theory is decidable for finite signatures, NT19). **Refinement**, written `f ⊑ f′`, means `f′` preserves the behavior of `f` at no greater cost (NT28); maximal refinement of a behavior class is unique and its dispatcher is cost-minimal (NT29). **Minimization** replaces M by its minimal-state realization `S_min(M)`, unique up to isomorphism, behavior-preserving, with compositional state bounds (NT38–NT41).

**Rings and buffers.** A *ring* is a bounded, first-in-first-out buffer with `push` and `pop` operations that obey the ring equations `push ∘ pop = id` and `pop ∘ push = id` (on a non-empty ring) in every model (NT9); the ring rewrite system is terminating, locally confluent, and complete (NT13–NT15), so ring traffic can be normalized. A ring of capacity `2^k` indexed by bitmask is correct iff the number of in-flight elements is at most `2^k − 1` (NT48). A *buffer* is any preallocated, bounded storage region used by a data path. Rings and buffers are allocated and sized before the hot path begins and are never resized on it (policy R-01).
...
The **Situation Application Matrix** is the default per-situation mask. Its rows are policies; its columns are the five situation kinds; each cell holds the effective level — MUST, SHOULD, MAY, or "—" (not applicable) — of that policy for a situation of that kind.

For a situation `s` of kind `K` and a policy `P`:

- if `Scope(P)(s)` is false, the effective level is **—** (not applicable), whatever the cell says;
...
Cells use MUST, SHOULD, MAY, and "—" (not applicable). Rows are policy identifiers as defined in Section 5; the Level fields there restate the scope-qualified reading of these rows.

**Table 4-1 — Default mask, policies [A] through [CACHE]**

| Policy | D | C | U | S | O |
|---|---|---|---|---|---|
| A-01 Atom purity classification | MUST | MUST | MUST | MUST | — |
| A-02 No hidden state | MUST | MUST | MUST | MUST | — |
| A-03 Totality on declared domain | MUST | MUST | MUST | MUST | — |
| A-04 Domain declaration | MUST | SHOULD | SHOULD | SHOULD | — |
| MOL-01 Composition by ∘ and ⊗ | MUST | MUST | — | — | — |
| MOL-02 No dynamic dispatch on the hot path | MUST | — | — | — | — |
| MOL-03 Dense-integer dispatch | MUST | — | — | — | — |
| MOL-04 Minimal state | MUST | SHOULD | — | — | — |
| MOL-05 Behavioral specification declared | MUST | MUST | MUST | — | — |
| R-01 Preallocation before the hot path | MUST | SHOULD | SHOULD | — | — |
| R-02 Power-of-two capacity, bitmask indexing | MUST | SHOULD | — | — | — |
| R-03 Bounded in-flight invariant | MUST | MUST | — | SHOULD | — |
| R-04 Ring laws hold in every model | MUST | MUST | — | — | — |
| ALLOC-01 Zero allocation in declared hot paths | MUST | — | — | — | — |
| ALLOC-02 Statically sized or preallocated collections | MUST | SHOULD | SHOULD | — | — |
| ALLOC-03 Initialized memory | MUST | MUST | MUST | MUST | — |
| ALLOC-04 Capacity-bounded allocation, defined overflow | MUST | MUST | SHOULD | SHOULD | — |
| CACHE-01 Cache-line alignment | MUST | SHOULD | — | — | — |
| CACHE-02 Hot/cold separation | MUST | — | — | — | — |
| CACHE-03 False-sharing avoidance | MUST | — | — | — | — |
| CACHE-04 Layout justified by access pattern | MUST | SHOULD | — | — | — |

**Table 4-2 — Default mask, policies [SIMD] through [INT]**

| Policy | D | C | U | S | O |
|---|---|---|---|---|---|
| SIMD-01 Vector ops never out of bounds | MUST | MUST | — | — | — |
| SIMD-02 Alignment satisfied | MUST | MUST | — | — | — |
| SIMD-03 Feature-gated with fallback | MUST | MUST | — | — | — |
| IO-01 Nonblocking data-path I/O | MUST | — | — | — | — |
| IO-02 Drain-until-exhausted | MUST | — | — | — | — |
| IO-03 Batching to amortize syscalls | MUST | MAY | — | — | — |
| IO-04 Options justified by targets | MUST | SHOULD | SHOULD | — | — |
| CONC-01 Per-core state preferred | MUST | SHOULD | — | — | — |
| CONC-02 Lock-free primitives only on the data path | MUST | SHOULD | — | — | — |
| CONC-03 Memory orderings justified | SHOULD | MAY | — | — | — |
| CONC-04 Concurrency model declared | MUST | MUST | SHOULD | — | — |
| SEC-01 Length-validated before use | MUST | MUST | — | — | — |
| SEC-02 Truncation detected | MUST | MUST | MUST | MUST | — |
| SEC-03 No uninitialized reads at trust boundaries | MUST | MUST | — | — | — |
| SEC-04 Resource caps declared | MUST | MUST | SHOULD | — | — |
| SEC-05 Dependency audit | MUST | MUST | MUST | MUST | MUST |
| OBS-01 Lock-free counters | MUST | MAY | — | — | — |
| OBS-02 Pull-based metrics | MUST | MAY | — | — | — |
| OBS-03 Zero-cost when compiled out | SHOULD | MAY | — | — | — |
| PLUGIN-01 ABI versioning | MUST | MUST | MUST | — | — |
| PLUGIN-02 Health checks | SHOULD | SHOULD | SHOULD | — | — |
| PLUGIN-03 Safe fallback | MUST | SHOULD | — | — | — |
| TEST-01 Property-based law tests | — | — | — | — | MUST |
| TEST-02 Fuzz for parsers and input handlers | — | — | — | — | MUST |
| TEST-03 Differential tests against a reference | — | — | — | — | MUST |
| TEST-04 Cost and zero-allocation regression | — | — | — | — | MUST |
| INT-01 Policies bind resources, invariants, behavior — never architecture | MUST | MUST | MUST | MUST | MUST |
...
For the [TEST] policies the runtime cells (D–S) are marked "—" because the obligations execute in the offline (CI) context; their Scope states which situations' molecules the obligation covers — a data-path molecule's law tests run in CI, not in the data path. The [INT] policies bind everywhere because they are the standard's contract with application authors, asserted as standing review questions in every review.
...
**Rationale.** The thesis splits Mol into PureMol (total functions, NT4) and EffMol(Ctx) (the Kleisli category of the state monad, NT5), and proves that every hybrid molecule factors uniquely as `q ∘ e ∘ p` with p and q pure and e effectful (NT33). The classification is what makes that decomposition possible, and with it the zero-allocation data path (NT50) and the determinism of pure fragments (NT8). An atom that is both pure and effectful has no home in the decomposition and no decidable behavior.
...
**Rationale.** The behavior of a molecule is determined by its `(S, step)` (NT1). Hidden state makes the real state space larger than the declared one, which breaks the quotient that defines Mol (NT2), invalidates the minimal-state bounds (NT40), and destroys the determinism of pure fragments (NT8). Hidden state is how determinism and testability leak; it is also invisible to the cost model, so it can exceed the memory budget `m̄` without being charged (NT25).
...
**Rationale.** PureMol is the category of total functions (NT4); totality is what makes a transformation a function at all, and what makes the cost bounds of NT25–NT26 hold over the entire declared domain rather than over a sampled subset. An atom that silently returns garbage, crashes, or reads out of bounds on a plausible input breaks `M ⊢ Φ` for every situation that uses it (NT51).
...
**Rationale.** The declared domain is what totality (A-03) and coverage (TEST-01 ff.) are measured against, and the situation semantics of the thesis require a decidable description of what the solution must accept (NT51). Without a declared domain, no atom-level guarantee can be stated or checked, and the feasible set of solutions is not a set at all.
...
**Rationale.** Composition in Mol is associative (NT1), respects behavioral equivalence (NT6), and lifts refinement with cost monotonicity (NT28). The tensor product makes independent parts explicit — the basis of per-core partitioning (CONC-01) and of parallelism claims (NT30). Ad-hoc wiring has none of these properties: it is invisible to the type graph, so its behavior and its cost are both outside the decidable theory (NT19, NT51).
...
**Rationale.** The thesis proves that the most-refined dispatcher is sound and cost-minimal (NT29) and that normal forms form precomputable lookup tables (NT23–NT24); statically resolved, branch-predictable composition is what makes the fixed-iteration bounds of NT43 and the cost bounds of NT25–NT26 hold in practice. Dynamic dispatch on the hot path adds an unpredictable indirection whose cost is invisible to the type graph and therefore unaccounted in `c̄`.
...
**Rationale.** A dense key is a finite signature; the thesis' normal-form tables are exactly dense lookup tables over finite signatures (NT23–NT24), and the most-refined dispatcher is the cost-minimal one (NT29). Chained conditionals scale the branch cost linearly in the number of alternatives and defeat branch prediction on wide distributions; a dense table makes dispatch a single indexed step with a precomputable, bounded shape.
...
**Rationale.** Every finite-state molecule has a minimal state space unique up to isomorphism (NT38), minimization preserves behavior (NT39), compositional state bounds hold (NT40), and minimization is compatible with behavioral equivalence (NT41). Because the cache footprint of loop state is monotone in state size (NT37), smaller state is faster state; redundancy also enlarges the search space of the decision problem (NT26).
...
**Enforcement.** M3 — property tests assert behavioral equivalence between the molecule and its reduced version (bisimulation, NT6); M4 — review examines the state declaration for redundant fields.
...
**Rationale.** A solution to a situation is a molecule with `M ⊢ Φ` (NT51). An unnamed Φ makes "solution" undecidable; a named Φ makes the feasible set finite and decidable (NT51) and equivalence checkable by normal forms (NT24).
...
**Rationale.** The linearity theorem (NT50) makes a well-typed hot path allocation-free; preallocation is its construction-level realization. Allocation on the hot path re-introduces the very costs — boundedness, latency, cache — that NT25–NT26 budget against, and turns a static capacity into a runtime question.
...
**Rationale.** NT48: a ring of capacity `2^k` with bitmask indexing is correct iff the number of in-flight elements is at most `2^k − 1`; the bitmask replaces a modulus, making the index step a single machine operation and the capacity test a single compare — costs that belong in the per-message budget.
...
**Rationale.** This is the correctness half of NT48 (bitmask indexing is correct iff in-flight ≤ `2^k − 1`), and the ring equations `push ∘ pop = id` and `pop ∘ push = id` (NT9) hold only when pop never observes an empty ring and push never overwrites a live slot. Silent overwrite makes the ring a lossy channel whose behavior is not a function of its input — a violation of totality (A-03) at the molecule level.
...
**Rationale.** The ring theory is complete for the free ring model (NT9), its rewrite system is terminating and confluent (NT13–NT15), and rewriting by cost-nonincreasing rules preserves behavior and does not raise cost (NT18). These laws are what allow the runtime and the builder to normalize ring traffic.
...
**Rationale.** Static bounds make maximum consumption a compile-time or load-time property — the memory budget `m̄` of the situation (NT25) — and a collection whose size is unknown is a cost that cannot enter the budget vector and cannot be optimized over (NT26). Boundedness is the affine/resource discipline of ARCSS adopted here because boundedness is what keeps `m̄` decidable (NT51).
...
**Rationale.** Reading uninitialized memory is undefined behavior; a molecule that can do so is not a total function on its declared domain (A-03, NT4), and its behavior is not a function of its input at all. The determinism guarantee for pure molecules (NT8) and the equivalence notion (NT6) presuppose defined values.
...
**Rationale.** Boundedness makes consumption a static property (ALLOC-02, NT25) and keeps the cost vector decidable (NT51). Unbounded allocation influenced by untrusted input is a denial-of-service vector (SEC-04).
...
**Rationale.** Cache-line alignment prevents a structure from straddling lines (which doubles the transfers per access) and is a precondition for vector access (SIMD-02). The cost vector `c̄` counts cache misses (NT25); alignment is a static way of spending that budget well, and the cache-footprint monotonicity of NT37 makes compact hot state pay directly.
...
**Rationale.** The cache footprint of loop state is monotone in state size (NT37), and hot/cold splitting is the layout form of state minimization (MOL-04): it shrinks the hot working set to what the hot path actually reads. Cold fields dragged into hot lines waste the miss budget (NT25).
...
**Rationale.** False sharing makes two independent molecules — tensor factors, NT3 — pay for each other's cache traffic; it converts the per-core independence that CONC-01 establishes into hidden contention. The cost is a miss-budget tax invisible to the type graph (NT25) that violates the independence assumptions of the per-core decomposition (NT30).
...
**Rationale.** Layout is the concrete realization of the cost vector (NT25): for a given access trace, the layout determines which accesses miss. An unexamined layout is an unexamined cost. The thesis' quantitative semantics make layout a first-class optimization target (NT25–NT26); the standard's job is to make the choice explicit and checkable.
...
**Rationale.** A vector op is a tensor of scalar ops (NT11 — batching distributes), and the scalar remainder is what makes the whole a total function on the declared range (A-03, NT4). An out-of-bounds vector access is undefined behavior and breaks totality exactly as an unchecked scalar read does.
...
**Rationale.** Misaligned vector access is undefined behavior or a fault; it violates totality (A-03). The alignment check is a pure predicate that selects between the vector and scalar branches — the same refinement structure as feature gating (SIMD-03; NT28).
...
**Rationale.** Feature gating is the common-case specialization structure of NT28 (refinement substitution): the vector path and the scalar path both refine the same behavior, and selection is sound because behavior is preserved (NT6). The cost difference is exactly what the cost vector wants (NT25): the same behavior at lower cost on capable hardware, correct behavior everywhere.
...
**Rationale.** Blocking on the data path makes the per-message cost unbounded — a cost that cannot enter `c̄` (NT25) and a schedule that defeats the batching laws (NT47). Nonblocking I/O keeps the data-path molecule a function of its ready inputs (A-03, NT4), with latency bounded by the budget.
...
**Rationale.** Draining is the I/O form of batching: each drain amortizes the syscall/event cost over all available work (NT47). Draining one item per event multiplies the fixed cost per message and can starve the machinery with event churn.
...
**Rationale.** NT47: `cost(batch_n) ≤ n · cost(single) + fixed(n)`, with `fixed(n)/n → 0`. The syscall cost is the fixed part; batching amortizes it over n messages. Batching is also the concrete form of the batch laws (NT11, NT46) that make batch trees flatten.
...
**Rationale.** Per-core partitioning is the tensor structure of the situation (NT3, NT30): independent molecules compose without interference (NT8), and partitioned state keeps each core's working set cache-resident (NT37). Shared mutable state, by contrast, reintroduces interference and destroys the per-core cost model (NT25).
...
**Rationale.** Lock-free primitives keep the per-operation cost bounded and independent of contention patterns, so the cost stays in `c̄` (NT25); they preserve the determinism of pure fragments (NT8). Locks on the data path make latency a function of other threads' behavior — an unbounded interference that no budget can contain.
...
**Rationale.** Ordering strength is a cost on the data path (fences and barriers stall the pipeline) and a correctness property at the same time; an unexamined ordering either overpays (NT25 budget) or under-delivers (breaking the invariant, NT6). The weakest-correct-ordering discipline keeps the cost vector honest.
...
**Rationale.** Truncation silently changes the declared domain of a value — a length that becomes small after narrowing is no longer the length it was — breaking totality (A-03) and turning a security check into a false one. The checked alternative keeps the molecule a total function (NT4).
...
**Rationale.** Caps are the situation's bounded-resource declaration: they make the cost vector decidable (NT51) and the ring invariant checkable (NT48). Unbounded resources are the classic exhaustion attack and make the memory budget `m̄` meaningless (NT25).
...
**Rationale.** Observability is a tensor factor of the molecule (NT3): it must not interfere with the measured behavior (NT8) or its cost budget (NT25). A locking metric would turn measurement into a perturbation; the measurement cost must stay in `c̄` like any other cost.
...
**Rationale.** Push from the data path converts a bounded data-path cost into an unbounded one (the sink may be slow); pull keeps all data-path costs bounded and in `c̄` (NT25). This is the observability form of the bounded-in-flight discipline (R-03): the sink is a consumer the data path never waits for.
...
**Rationale.** NT18 — normalization removes steps with behavior preserved and cost not increased when the rules are cost-nonincreasing. Metrics that change behavior — a metric that allocates, or whose absence changes control flow — would violate this; metrics that do not can be compiled out exactly as the theorem describes.

**Wrong.** Metrics whose collection affects timing-dependent behavior, or whose removal changes code paths.

**Correct.** Metrics are pure side-records (counter updates) that compile away; tests run with metrics on and off and assert behavioral equivalence (bisimulation, NT6).
...
**Rationale.** A plugin is a molecule composed into the host across an external boundary; the interface is the signature Σ of the composition (NT20–NT21). Versioning makes the signature decidable and the equivalence classes of plugins finite and checkable (NT22–NT24, NT19): two plugins with the same versioned interface are comparable; a mismatched one is out of the signature.
...
**Rationale.** The host composes the plugin into a molecule whose behavior must satisfy Φ (NT51); a plugin that has drifted from its contract (NT6) makes the composite's behavior undetermined. A health predicate is the decidable proxy: the host verifies that the plugin still refines its interface (NT28) without trusting it.
...
**Rationale.** Fallback is the refinement substitution of NT28 — the built-in refines the same Φ at known cost — and fail-closed keeps the composite's behavior within Φ (NT51). A data path that hard-depends on an unchecked plugin makes Φ undecidable at run time.
...
**Statement.** Every molecule ships property-based tests of its equational laws: ring laws (`push ∘ pop ≈ id`; `pop ∘ push ≈ id` on non-empty rings), batch laws (flattening, NT46), parser laws (left-factoring normal forms, NT16), transport interchange (NT11), and any law the molecule's theory declares. Tests are generated over the declared domain and its boundaries.

**Rationale.** The laws are the specification: they are complete for their theories (NT9, NT16, NT17), and normalization is sound (NT18). Property tests that check the laws on randomized inputs check exactly the identities the runtime and the builder rely on (NT24).
...
**Rationale.** Parsers sit at the trust boundary (SEC-01); totality on the declared domain (A-03) is exactly what fuzzing checks, and NT16's left-factoring gives fuzzing a well-formed target — the normal-form parser. Fuzzing is the empirical complement to the domain partition (A-04).
...
**Rationale.** Differential testing checks `M ≈ M_Φ` (NT6): behavioral equivalence, the congruence that makes equivalence decidable by normal forms (NT24). It is the direct test of the solution predicate (NT51).
...
**Rationale.** The cost vector `c̄` is part of the situation (NT51); a regression that silently exceeds it changes the feasible set — the molecule may no longer be a solution. NT18 (cost-nonincreasing normalization) and NT50 (allocation-free hot paths) give the checks their grounding: these are properties, not aspirations, so they can be asserted in CI.
...
**Rationale.** NT48 (bitmask ring correct iff in-flight ≤ K − 1); NT37 (cache footprint of state is monotone in state size — the ring should stay L3-resident); NT47 (batch volume couples the ring and batch budgets).
...
**Rationale.** NT25 (cache misses are components of `c̄`; layout determines the miss rate); NT30 (SoA is the tensor of the field morphisms, and the tensor structure is closed under composition).
...
**Rationale.** NT25–NT26 (the cost vector decides: the protocol fixes the per-message fixed-cost floor; choose the feasible protocol with the lowest floor); NT47 (batching interacts: datagram protocols batch natively, stream protocols batch records within the stream).
...
**Decision rule.** Maximize n such that `n·t_m + t_s ≤ L` and `n·μ ≤` the buffer budget. Stop increasing n once the marginal gain — `fixed(n)/n`, the amortized fixed cost — falls below a recorded threshold (NT47's diminishing returns). n ≥ 1 always. Record n, the measurement, and the threshold.

**Rationale.** NT47: `cost(batch_n) ≤ n · cost(single) + fixed(n)`, with `fixed(n)/n → 0`; the optimal n is where the amortized fixed cost is dominated by the per-message cost, subject to the latency budget.
...
**Rationale.** NT47 (amortization: kernel-side batching reduces the fixed cost per batch); NT52 (zero-copy roundtrip elimination is sound only when the domain is well-formed and the transformation is invertible — see D-6).

**Output.** IO-01, IO-02, IO-03 at MUST for data-path situations (mask rows); the choice is documented under IO-04.

### D-6 — Zero-Copy vs Copy (NT52)
...
**Rationale.** NT29 (the most-refined dispatcher is sound and cost-minimal); NT23–NT24 (normal forms are precomputable dense tables over finite signatures).
...
**Rationale.** NT11 (a vector op is a tensor of scalar ops; the batch law distributes); NT4 (totality on the declared range — the scalar remainder completes the tuple).
...
**Rationale.** NT8 (independent tensor factors are deterministic — partitioning removes interference); NT25 (interference must not enter the budget — locks on the data path make cost a function of other threads); NT30 (the per-core decomposition is closed under composition).
...
**Rationale.** NT37 (cache footprint of loop state is monotone in state size); NT25 (the miss budget is a component of `c̄`).
...
**Rationale.** NT50 (linearity ⇒ a well-typed hot path never allocates or deallocates); NT26 (the cost-minimal design is a shortest path in the finite type graph — an unbounded allocation site is outside the graph); NT48 (rings need fixed capacity to be correct).
...
**Rationale.** NT28 (refinement substitution: the fallback refines the same Φ at known cost); NT22–NT24 (a versioned signature makes the plugin interface decidable and comparable).
...
1. `M ⊢ Φ` — M satisfies the specification, equivalently `M ≈ M_Φ` where `M_Φ` is the canonical molecule of Φ (behavioral equivalence, NT6, decided by normal forms, NT24); and
2. `cost(M) ≤ c̄` — componentwise cost dominance, where costs are additive over composition and subadditive over tensor (NT25).

The **feasible set** — all molecules satisfying 1 and 2 — is finite and decidable for bounded types and additive costs (NT51). The decision problem "find the solution of minimal cost" is therefore a finite search: a shortest-path computation over the finite type graph (NT26). The decision matrices of Section 6 are the practical form of that minimization: each matrix restricts the search to the designs consistent with the situation's binding levels, and the conjunction of the matrices selects the point in the feasible set that the situation record documents.
...
- *(Conn, Request → Response)* — data path: per-connection ring (D-1), HTTP parser (A-04 domain partition: well-formed / truncated / trailing-garbage), request molecule, response serializer, write batching (D-4). Keep-alive pipelines requests into the batch (NT46 batch flattening).
- *(Idle → Close)* — control path: teardown, draining (R-03 at shutdown).

**Molecules.** The per-request molecule factors as `q ∘ e ∘ p` (NT33): `p` parses and validates the request (pure; fuzzed, TEST-02; length caps, SEC-01); `e` performs I/O through Ctx (nonblocking, IO-01; drains, IO-02; batches, IO-03); `q` serializes the response (pure; the inverse of `p` on the well-formed domain, NT52 — a serialization roundtrip in a proxy situation is eliminated, D-6). State: per-connection ring and parse state, minimal (MOL-04). Dispatch on method codes is dense-integer (MOL-03); the parser's choice structure follows left-factored normal forms (NT16).
...
**Situations.**
- *(Query packet → Response packet)* — data path over UDP: datagram parse (SEC-01: query size cap, name-length caps, compression-pointer bounds — a parser law, NT10/NT16), cache lookup (bounded cache, ALLOC-02), response build, multi-message batching (IO-03, D-4), ring of outstanding queries (R-02/R-03, D-1).
...
- **Behavioral equivalence (≈)** — bisimulation: two molecules are equivalent when they produce indistinguishable input/output behavior; a congruence for ∘ and ⊗ (NT6); decidable by normal forms (NT24).
...
- **Ctx** — the effect context of an effectful atom or molecule: the set of capabilities (I/O handles, clocks, allocation) it may touch and nothing else. See NT5, A-01.
...
- **Cost budget vector c̄** — `c̄ = (t̄, m̄, k̄)`: budgets for time, memory, and cache misses that a solution must meet componentwise (NT25, NT51).
- **Data path** — the steady-state per-message or per-batch execution of a data-path situation; the hot path. See Section 2.
- **Decision matrix** — a procedure (Section 6) that resolves a design choice within a situation, from recorded inputs; formally, the minimization over the feasible set (NT26, NT51).
...
- **Effectful atom / effectful molecule** — an atom/molecule that acts through Ctx (NT5). See A-01.
- **Feasible set** — the set of molecules satisfying `M ⊢ Φ` and `cost(M) ≤ c̄`; finite and decidable for bounded types and additive costs (NT51).
...
- **Hybrid molecule** — a molecule mixing pure and effectful steps; uniquely `q ∘ e ∘ p` (NT33).
- **In-flight** — elements placed in a ring but not yet removed; bounded by `capacity − 1` (NT48, R-03).
- **Mask (Situation Application Matrix)** — the per-situation default assignment of effective policy levels (MUST/SHOULD/MAY/—) by situation kind; overridable only with written justification (Section 4.4).
- **Molecule** — a stateful transformation `M : A → B`; an equivalence class of `(S, step)` pairs (NT1–NT2). See Section 2.
- **Normal form NF(M)** — the unique (up to coherence) normal form of M under the runtime's equational theory; `NF(M) ≈ M`, and equivalence is decided by comparing normal forms (NT23–NT24).
...
- **Pure atom / pure molecule** — a total function with no observable effects (NT4); deterministic (NT8).
- **Refinement (⊑)** — `f ⊑ f′`: f′ preserves the behavior of f at no greater cost (NT28); maximal refinement is unique and its dispatcher cost-minimal (NT29).
- **Ring** — a bounded FIFO with `push`/`pop` obeying the ring equations (NT9); power-of-two capacity with bitmask indexing when `2^k` (NT48). See [R].
...
- **Solution** — a molecule M with `M ⊢ Φ` and `cost(M) ≤ c̄` (NT51).
- **State space S** — the complete declared memory of a transformation; `step : S × A → B × S`. See Section 2.
- **Step function** — the function `step : S × A → B × S` of a stateful transformation (NT1).
- **Tensor product (⊗)** — parallel composition of molecules; the tuple product with unit `()` (NT3).
...
- **Zero allocation** — a declared hot path performing no allocation or deallocation; grounded in NT50 and enforced by TEST-04.
...
- `UDP_GRO` (generic receive offload) — receive coalescing of related datagrams; reduces per-packet fixed cost, interacts with the parser laws (a coalesced payload is several datagrams, which the parser must still handle one-by-one — NT11).
- `SO_ZEROCOPY` — send-side zero copy; relevant to D-6 (zero-copy is sound when the data passes through untouched or the roundtrip is invertible, NT52).
...
- **nom** — a combinator parser library; a candidate basis for custom-protocol parsers (Recipe 7.4), whose laws (NT10/NT16) are then tested by TEST-01.
