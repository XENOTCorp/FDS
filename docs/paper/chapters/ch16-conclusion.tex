...
\ref{nt:54} & [N] & Modular completion: adjoining a new equational theory preserves termination and confluence under the disjoint-union or hierarchical conditions; decidability is preserved. & \autoref{ch:rewrite}\\
\bottomrule
\end{tabular}
\end{table}
...
\ref{nt:53} & [N] & Resource-graded cost enrichment: cache-line, false-sharing, and expected branch-misprediction costs form a graded monoidal enrichment; shortest-path optimality extends. & \autoref{ch:enrich}\\
\bottomrule
\end{tabular}
\end{table}
...
\ref{nt:55} & [N] & Affine typing: well-typed molecules consume resources exactly once and evaluate without allocation. & \autoref{ch:linear}\\
...
\textbf{Machine-checked formalization.} The entire catalog NT1--NT55 is a candidate for a proof assistant. The elementary character of the proofs and the decidable normal forms (\ref{nt:19}, \ref{nt:24}) suggest that a large fraction of the catalog is formalizable automatically, with the rewrite theorems (\ref{nt:13}--\ref{nt:19}) as the natural starting point.
