...
particular machine. On this we establish the category structure (NT1--NT2),
the symmetric monoidal structure (NT3), the subcategories of pure, effectful,
and hybrid molecules (NT4--NT5), behavioral equivalence as a congruence (NT6),
closure under the tensor (NT7), and determinism (NT8). Throughout, types are
...
$g \circ f$ is independent of the chosen representatives is \autoref{nt:2}.
\end{definition}
...
\begin{theorem}[Mol is a category (NT1)]\label{nt:1}
\textbf{[N].} Composition of $(S, \mathrm{step})$ classes is associative with
identity: Mol is a category.
\end{theorem}

\begin{proof}
Composition on classes is well defined by \autoref{nt:2}, proved below; given
...
\begin{theorem}[The bijection quotient is well defined (NT2)]\label{nt:2}
...
\begin{remark}
With \autoref{nt:2} in place, composition is a genuine function on the
hom-sets of Mol, and \autoref{nt:1} makes Mol a category. The same
...
\begin{theorem}[Mol is symmetric monoidal (NT3)]\label{nt:3}
...
two isomorphism hypotheses, exactly as in the proof of \autoref{nt:2}. The
...
composition is by \autoref{nt:2}, the coherence equations hold on classes
...
\begin{theorem}[PureMol is the category of functions (NT4)]\label{nt:4}
...
--- machines with state space $\mathrm{Ctx}$ --- compared by literal equality
...
\begin{theorem}[EffMol is the Kleisli category of the state monad (NT5)]\label{nt:5}
...
it all pure morphisms, which the identity laws of \autoref{nt:1} require.
...
being a state-space isomorphism by the product case of \autoref{nt:2}; the
tensor closes by the same computation, absorbing the extra context copy into
the pure part, as recorded in \autoref{nt:7}. EffMol$(\mathrm{Ctx})$ sits
...
\begin{theorem}[Behavioral equivalence is a congruence (NT6)]\label{nt:6}
...
\autoref{nt:3} hold in Mol and are equations between morphisms, so they hold
in the quotient; the structural morphisms of \autoref{nt:3} remain
...
\begin{theorem}[Closure under the tensor (NT7)]\label{nt:7}
...
\begin{remark}
The exact boundary of \autoref{nt:7} is worth stating: the tensor of two
...
\begin{theorem}[Determinism (NT8)]\label{nt:8}
...
molecules is pure by \autoref{nt:7}, hence deterministic. The content of the
...
schemes of the dataplane. The category laws (NT1--NT2) license equational
reasoning about pipelines; the symmetric monoidal structure (NT3) licenses
...
behavioral equivalence (NT6) is the relation under which minimization
(\autoref{ch:coalgebra}) and testing (\autoref{ch:testing}) operate, while
determinism (NT8) is the assumption under which the feedback structure of
...
\midrule
\ref{nt:1} & [N] & Mol is a category: composition of $(S, \mathrm{step})$ classes is associative with identity. \\
\ref{nt:2} & [N] & The state-space bijection quotient is well defined: equivalent machines compose to equivalent machines. \\
\ref{nt:3} & [C] & Mol is symmetric monoidal: tensor the tuple product, unit $()$, coherent braiding \citep{maclane}. \\
\ref{nt:4} & [N] & PureMol is equivalent to the category of types and total functions. \\
\ref{nt:5} & [N] & EffMol$(\mathrm{Ctx})$ is isomorphic to the Kleisli category of the state monad on $\mathrm{Ctx}$. \\
\ref{nt:6} & [N] & Behavioral equivalence is a congruence for $\circ$ and $\otimes$; Mol$/{\approx}$ is symmetric monoidal. \\
\ref{nt:7} & [N] & PureMol and EffMol are closed under the tensor. \\
\ref{nt:8} & [N] & Pure molecules and tensor products of pure molecules are deterministic; behavioral equivalence preserves determinism. \\
\bottomrule
\end{tabular}
