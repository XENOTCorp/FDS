...
A component is not only a molecule; it is a molecule governed by equations.
...
(\autoref{nt:6}). The theories, their models in Mol, and the rules they
license are developed in \S\ref{sec:lawvere-ring}--\S\ref{sec:lawvere-rewrite},
with NT9--NT12 stated and proved in place.
...
of Mol (\autoref{ch:mol}), a symmetric monoidal category (\autoref{nt:3}).
Equations whose two sides realize genuinely different state spaces are
asserted in the behavioral quotient Mol/$\approx$ (\autoref{nt:6}); where
...
fixed-capacity array, indexed by bitmask (\autoref{nt:48}), into which
...
(\autoref{nt:48}), with $\mathit{push}$ defined while room remains. This is
...
complete (\autoref{nt:9}).
\end{remark}

\begin{theorem}[Ring theory equations (NT9) [N]]\label{nt:9}
...
of this semantics (\autoref{nt:10}).

\begin{theorem}[Parser theory (NT10) [N]]\label{nt:10}
...
\begin{theorem}[Transport theory (NT11) [N]]\label{nt:11}
...
(\autoref{nt:3}, \cite{maclane}) in pipeline order.
\end{proof}
...
of \autoref{ch:rewrite} (NT13--NT19) turn the rules into a decision
...
\begin{theorem}[Ring cancellation (NT12) [N]]\label{nt:12}
