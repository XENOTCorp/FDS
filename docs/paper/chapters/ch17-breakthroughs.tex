...
\begin{theorem}[Boundary enrichment is well-defined]\label{thm:boundary}
...
\begin{theorem}[Batch fusion]\label{thm:batch}
...
\begin{theorem}[The affine crossover]\label{thm:crossover}
...
\begin{theorem}[The optimization lattice]\label{thm:lattice}
...
$\mathrm{NF} (M)$ is a finite lattice under the componentwise cost
order, and it has a least element: the realization of minimal boundary
count, and among those, minimal cost;
...
$\mathrm{NF} (M)$ is a candidate; the boundary count and cost vector are
each $\mathbb{N}$-valued on a finite set, so the lexicographic minimum
exists and is found by the finite enumeration. The enumeration
terminates because $\mathcal{R}$ is terminating and the signature is
finite: no infinite rewrite chains to new realizations beyond the
finite normal forms.
\end{proof}
...
\begin{theorem}[Bypass is a retraction]\label{thm:bypass}
...
\begin{theorem}[Cache soundness]\label{thm:cache}
...
\begin{theorem}[Band contraction]\label{thm:ema}
...
that is, $x \in B$.
(2): write $x = 8n + k$ with $k$ an integer. If $x < 8n$ then $k < 0$
and $x/8 \leq n-1$, so $F (x) = x + n - x/8 \geq x + n - (n-1) = x+1 > x$,
and $F (x) \leq x + n \leq 8n - 1 + n = 9n - 1$; since $x/8 \geq n-1$
is false only for $x < 8 (n-1)$, iterating eventually reaches the band
because each step increases $x$ by at least $1$ while $x < 8n$. If
$x > 8n+7$, then $x/8 \geq n+1$, so
$F (x) = x + n - x/8 \leq x + n - (n+1) = x - 1 < x$, and
$F (x) \geq x/8 \cdot 7 + n \geq 7 (n+1) + n = 8n+7$, so the iterate
stays above the band until it enters it; each step decreases $x$ by at
least $1$, so entry is finite.
...
\begin{theorem}[Zero-boundary loops]\label{thm:zero-boundary}
...
\begin{proof}
The affine typing forces exact-once consumption and no allocation; a
...
\begin{theorem}[Route languages are regular; the pattern trie is a deterministic automaton]\label{thm:route-dfa}
...
\begin{theorem}[The automaton is the Myhill--Nerode refinement]\label{thm:route-mn}
...
\begin{theorem}[Hybrid matcher selection]\label{thm:hybrid}
...
machine the measured order is: readiness-driven busy-poll first,
completion-driven second, completion-driven with a kernel polling
thread last. The winner is a lattice minimum: the finite set of
...
the answer to the question ``what is the last optimization?'' It is
not a bag of tricks; it is the decision procedure that
