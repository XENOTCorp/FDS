...
coherent isomorphism, through $F(\Sigma)$ (NT20, NT21). \emph{Finiteness:} for
a finite signature with bounded types and a bounded number of atoms, hom-sets
are finite (NT22). \emph{Normal forms:} the runtime rewrite system of
...
\mathit{NF}(M')$ (NT23, NT24). The pay-off is the \emph{precomputed lookup
...
the empty tuple (\autoref{ch:mol}, NT3); Mol is a \emph{strict} symmetric
monoidal category, since tensor is literal concatenation. The behavioral
equivalence $\simeq$ (NT6) is a congruence for $\circ$ and $\otimes$ and is
the relation for which the normal forms of this chapter are complete (NT24).
...
\begin{theorem}[Existence of $F(\Sigma)$]\label{nt:20}
\textbf{NT20 [C].} The free symmetric monoidal category $F(\Sigma)$ on an
...
symmetric monoidal categories (NT21), it is the free object.
...
$F(\Sigma)$. The two constructions coincide by the uniqueness in NT21.
\end{proof}

\begin{theorem}[Universal property of $F(\Sigma)$]\label{nt:21}
\textbf{NT21 [C].} Let $\mathcal{C}$ be a symmetric monoidal category with a
...
\emph{Factorization.} Mol is strict symmetric monoidal (NT3), so the atoms of
...
\begin{theorem}[Finiteness for bounded signatures]\label{nt:22}
\textbf{NT22 [N].} Let $\Sigma$ be a finite atom signature with bounded
...
forms exist (NT23), so the lookup table of NT24 is finite and precomputable.
\end{theorem}
...
introduces atoms (NT23), so the normal forms of molecules with at most $n$
atoms are a subset of this finite set; by NT24 they are in bijection with the
behavior classes.
\end{proof}

\begin{remark}[The atom bound is essential]\label{rem:nt22-bound}
...
universal property (NT21) applied to the model in $\mathbf{Set}$ with
...
count because every atom has positive cost (NT51). This is the bounded,
...
--- ring cancellation (NT12), parser left factoring (NT16), batch flattening
(NT17) --- read as local rewrites of a sub-diagram with fixed boundary, so
the system is a well-defined relation on the morphisms of $F(\Sigma)$.

\begin{theorem}[Unique normal forms]\label{nt:23}
\textbf{NT23 [N].} Let $R$ be the runtime rewrite system of
...
NT16; the measures of NT13, NT16, NT17 rule out infinite reductions.
\emph{Confluence.} The system is locally confluent: every critical pair joins
(NT14, NT16, NT17), and redexes of different theories do not overlap, their
...
\begin{theorem}[Completeness and the lookup table]\label{nt:24}
\textbf{NT24 [N].} Let $R$ be the runtime rewrite system of
\autoref{ch:rewrite} on the molecules over a signature $\Sigma$: terminating
and confluent (NT13--NT17) and complete for behavioral equivalence, in that
...
\cite{knuthbendix}; established for the runtime theories by NT15, NT18,
NT19). Then for all molecules $M, M'$ over $\Sigma$:
\[
...
(NT12, NT16--NT18), so $M \to^{*} \mathit{NF}(M)$ implies $M \simeq
...
\emph{Lookup table.} By NT22 the normal forms of bounded molecules are finite
in number, and the two completeness directions make $M \mapsto \mathit{NF}(M)$
a bijection between behavior classes and normal forms; hence the table ---
one cost-optimal molecule per class (NT26, NT29), indexed by the type pair
...
type pair $A \to B$ its finitely many behavior classes (NT22), each
represented by its normal form (NT23), which is exactly its behavior (NT24);
with the costs of \autoref{ch:enrich} the stored representative is
cost-optimal within its class (NT26, NT29). Using the table is a lookup
...
Each row is the unique normal form of its behavior class (NT23, NT24), so the
table is consistent by construction; syntactically distinct terms in a class
--- $h \circ (g \circ f)$ and $(h \circ g) \circ f$, or $f$ and
$f \circ \mathrm{id}_A$ --- share one entry: the fusion promised by cut
elimination (\autoref{ch:linear}, NT49) is, for bounded signatures, a table
...
the search to one candidate per class, computed once (NT51).

\begin{remark}[Summary]
$F(\Sigma)$ is the syntax of molecules: existence and the universal property
(NT20, NT21) make it the canonical interface between expressions and Mol;
finiteness (NT22) makes optimization exhaustive; unique normal forms (NT23)
and completeness (NT24) make it unambiguous, so every behavior class has
