...
The cost enrichment of \autoref{nt:25}--\autoref{nt:27} accounts for a
...
\ref{nt:26}--\ref{nt:27} survive the refinement.
...
\begin{theorem}[Resource-graded cost enrichment, NT53 [N]]\label{nt:53}
...
inequality.
\end{enumerate}
Consequently the shortest-path theorem \ref{nt:26} extends: with grades
...
whenever the type signature is bounded, as in \ref{nt:22}), solvable by
Dijkstra's algorithm with the extraction-order induction of \ref{nt:26}.
...
the refinement poset of \ref{nt:28}--\ref{nt:29} orders, so that the
