...
\textbf{NT54 [N].} Modular completion: adjoining a new equational theory preserves termination and confluence under the disjoint-union or hierarchical conditions. \\
...
\textbf{NT53 [N].} Resource-graded cost enrichment: cache-line, false-sharing, and expected branch-misprediction costs form a graded monoidal enrichment; shortest-path optimality extends. \\
...
\textbf{NT55 [N].} Affine typing: well-typed molecules consume resources exactly once and evaluate without allocation. \\
\bottomrule
\end{tabular}}
...
\textbf{Part I --- Foundations} (\autoref{ch:prelim}--\autoref{ch:kleisli}). \autoref{ch:prelim} fixes the categorical machinery: monoidal, symmetric monoidal, and traced monoidal categories \citep{maclane,joyalsv}; monads and Kleisli; coalgebras; Lawvere theories \citep{lawvere}; operads and clubs \citep{leinster,kelly}; effectus theory; linear logic \citep{girard}; locally presentable categories \citep{adamek}. \autoref{ch:mol} defines Mol and its subcategories (NT1--NT8); \autoref{ch:lawvere} models rings, parsers, and transports as Lawvere theories (NT9--NT12); \autoref{ch:rewrite} develops the rewrite engine, with Knuth--Bendix completion \citep{knuthbendix} and Newman confluence \citep{newman} (NT13--NT19, NT54); \autoref{ch:free} constructs free molecules and normal forms (NT20--NT24); \autoref{ch:enrich} proves cost optimality and refinement (NT25--NT32, NT53); \autoref{ch:kleisli} proves the decomposition $M = q\circ e\circ p$ (NT33--NT37).

\textbf{Part II --- Structure theorems} (\autoref{ch:coalgebra}--\autoref{ch:linear}). \autoref{ch:coalgebra}: minimal realization and compositional bounds \citep{nerode,hopcroft} (NT38--NT41). \autoref{ch:trace}: feedback and fixed-iteration loops, with Banach--Dobrushin contraction bounds \citep{banach,dobroushin} (NT42--NT45). \autoref{ch:operad}: operadic composition and batching (NT46--NT48). \autoref{ch:linear}: cut elimination as fusion, linearity as zero allocation (NT49--NT50, NT55).
