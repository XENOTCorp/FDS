...
//! (`crates/fds/src/config.rs`). Every field carries the rationale
...
static CORE_FIELDS: &[FieldDef] = &[
    ...
    FieldDef {
        ...
        trade_off: "More workers give more parallelism but per-core caches and queues must fit; oversubscription adds scheduling noise (see the measured p999 tails in Docs/wiki/).",
    },
    FieldDef {
        key: "stack_bytes",
        json_type: JsonType::Int,
...
/// Emit `Code/config.json` with engine defaults, except socket
...
pub(crate) fn generate_schema() -> String {
    ...
    let schema = json!({
        ...
        "description": "The single Code/config.json consumed by the fds engine. Every field is optional: missing fields fall back to the engine defaults (crates/fds/src/config.rs). Hardware-derived defaults are produced by fds-detect (D-1 socket-buffer sizing from measured L3; worker count follows logical CPUs).",
        "type": "object",
