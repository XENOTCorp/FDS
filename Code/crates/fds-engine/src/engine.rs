...
fn worker_main(
    ...
    let udp_sock = fds::udp::UdpSocket::new(udp_addr, &cfg.udp)?;
    let tcp_listener = fds::tcp::TcpListener::bind(tcp_addr, &cfg.tcp, libc::SOMAXCONN)?;
    ...
    let mut metrics_server = match &metrics_path {
        Some(p) => Some(fds::metrics::MetricsServer::bind(p)?),
        None => None,
    };
    ...
    match cfg.reactor.strategy {
        fds::config::ReactorStrategy::EpollBusyPoll => worker_epoll_loop(
            id,
            cfg,
            &udp_sock,
            &tcp_listener,
        ...
        fds::config::ReactorStrategy::IoUring => {
...
fn worker_epoll_loop(
    id: usize,
    cfg: &Config,
    udp_sock: &fds::udp::UdpSocket,
    tcp_listener: &fds::tcp::TcpListener,
    metrics_server: &mut Option<fds::metrics::MetricsServer>,
    metrics: &Metrics,
    stop: &(dyn Fn() -> bool + Send + Sync),
) -> std::io::Result<()> {
...
/// ([`fds::io_uring_reactor::IoUringDatapath`]); UDP/TCP echo runs
...
fn startup_probes(cfg: &Config) {
    #[cfg(feature = "sctp")]
    {
        match fds::sctp::SctpSocket::bind("127.0.0.1:0".parse().unwrap(), &cfg.sctp) {
    ...
    #[cfg(feature = "io-uring")]
    {
        match fds::io_uring_reactor::IoUringReactor::new(8, 0) {
...
mod tests {
    ...
    fn engine_io_uring_echo() {
        let mut cfg = Config::default();
        cfg.reactor.strategy = fds::config::ReactorStrategy::IoUring;
        cfg.core.pin_cores = false;
        cfg.core.threads = 2;
        cfg.core.stack_bytes = 1 << 20;
