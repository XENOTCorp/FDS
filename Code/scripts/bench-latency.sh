...
sudo -n pkill -x nginx 2>/dev/null || pkill -x nginx 2>/dev/null
pkill -x atomos h2o caddy httpd 2>/dev/null; sleep 0.4
...
sudo -n pkill -x nginx 2>/dev/null || true; sleep 0.3
sudo -n bash -c "
...
SEASTAR_HTTPD="${SEASTAR_HTTPD:-}"
if [[ -n "$SEASTAR_HTTPD" && -x "$SEASTAR_HTTPD" ]]; then
  ("$SEASTAR_HTTPD" --smp 4 --port 8093 --load-balancing-algorithm connection_distribution --prometheus_port 0 >/tmp/seastar-lad.log 2>&1 &)
  ...
  echo "== seastar httpd skipped (set SEASTAR_HTTPD to the httpd binary) =="
fi

# --- atomos (default build) ---
pkill -x httpd 2>/dev/null; sleep 0.3
ATOMOS="${ATOMOS:-$ROOT/../../Atomos}"
if [[ -x "$ATOMOS/target/release/atomos" ]]; then
  (cd "$ATOMOS" && exec ./target/release/atomos --bind 127.0.0.1:8091 --root $WORK/root --rules examples/first_app/rules.json >/tmp/atomos-lad.log 2>&1 &)
  sleep 1.2
  echo "== atomos H1 =="
  ...
  echo "== atomos H1 skipped (ATOMOS tree not built) =="
fi
