#!/usr/bin/env bash
# perf.sh: profiling wrapper for the `fds` binary (crate fds-engine).
#
...
case "${1:-}" in
bench)
    echo "perf.sh: cargo run --release -p fds-engine -- --bench $SECONDS"
    cargo run --release -p fds-engine -- --bench "$SECONDS"
    ;;
stat)
    echo "perf.sh: perf stat -e cache-misses,cache-references,instructions,cycles,branch-misses -- cargo run --release -p fds-engine -- --bench $SECONDS"
    perf stat -e cache-misses,cache-references,instructions,cycles,branch-misses \
        cargo run --release -p fds-engine -- --bench "$SECONDS"
    ;;
record)
    echo "perf.sh: perf record -g -- cargo run --release -p fds-engine -- --bench $SECONDS"
    perf record -g -- cargo run --release -p fds-engine -- --bench "$SECONDS"
...
asm)
    ...
    echo "perf.sh: cargo asm -p fds-engine --bin fds --release $symbol"
    cargo asm -p fds-engine --bin fds --release "$symbol"
    ;;
llvm-mca)
    file="${2:-}"
    ...
    ;;
cachegrind)
    echo "perf.sh: valgrind --tool=cachegrind -- cargo run --release -p fds-engine -- --bench $SECONDS"
