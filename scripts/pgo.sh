...
RUSTFLAGS="$COMMON -C profile-generate=$PGO" cargo build --release -p fds-core
...
RUSTFLAGS="$COMMON -C profile-use=$PGO/merged.profdata" cargo build --release -p fds-core
